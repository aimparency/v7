...
function calculateCosts(aims, aimMap) {
    const costs = new Map();
    const calculating = new Set();
    function getCost(aimId) {
        if (costs.has(aimId))
            return costs.get(aimId);
        if (calculating.has(aimId))
            return 0; // Cycle breaking
...
function calculateDoneCosts(aims, aimMap, totalCosts) {
    const doneCosts = new Map();
    const calculating = new Set();
    function getDoneCost(aimId) {
        if (doneCosts.has(aimId))
            return doneCosts.get(aimId);
        if (calculating.has(aimId))
            return 0;
        calculating.add(aimId);
        const aim = aimMap.get(aimId);
