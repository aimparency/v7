...
function triggerRecalculation(projectPath) {
...
async function ensureProjectStructure(rawProjectPath) {
...
async function writeAim(rawProjectPath, aim) {
    const projectPath = normalizeProjectPath(rawProjectPath);
...
async function readAim(rawProjectPath, aimId) {
    const projectPath = normalizeProjectPath(rawProjectPath);
    ...
    if (aim.incoming && Array.isArray(aim.incoming)) {
        ...
        for (const incomingId of aim.incoming) {
            if (!aim.supportingConnections.some((c) => c.aimId === incomingId)) {
                newConnections.push({
                    aimId: incomingId,
...
async function listAims(rawProjectPath, archived = false) {
    ...
    if (!await fs.pathExists(aimsDir))
        return [];
...
function populateAimValues(projectPath, aims) {
    try {
...
async function writePhase(rawProjectPath, phase) {
    const projectPath = normalizeProjectPath(rawProjectPath);
    await ensureProjectStructure(projectPath);
...
async function cleanupCommitments(rawProjectPath, specificPhaseId) {
    const projectPath = normalizeProjectPath(rawProjectPath);
    const aims = await listAims(projectPath);
    let changedCount = 0;
    let validPhaseIds = null;
    if (!specificPhaseId) {
...
async function readSystemStatus(rawProjectPath) {
    const projectPath = normalizeProjectPath(rawProjectPath);
    ...
    const initialStatus = { computeCredits: 10.0, funds: 0.0 };
    await ensureProjectStructure(projectPath);
    await fs.writeJson(systemPath, initialStatus, { spaces: 2 });
    return initialStatus;
}
async function writeSystemStatus(rawProjectPath, status) {
    const projectPath = normalizeProjectPath(rawProjectPath);
    await ensureProjectStructure(projectPath);
    const systemPath = path.join(projectPath, 'system.json');
...
async function commitAimToPhase(projectPath, aimId, phaseId, insertionIndex) {
    // Update the phase
...
async function removeAimFromPhase(projectPath, aimId, phaseId) {
    // Update the phase
    const phase = await readPhase(projectPath, phaseId);
...
async function connectAimsInternal(projectPath, parentAimId, childAimId, parentIncomingIndex, childSupportedAimsIndex, relativePosition) {
...
async function migrateCommittedInField(projectPath) {
    ...
    aim: t.router({
        ...
        getRecursive: delayedProcedure
            ...
            while (queue.length > 0) {
                ...
                if (result.has(aimMap.get(aimId)))
                    continue;
                ...
                if (aim) {
                    ...
                    const connections = aim.supportingConnections || aim.incoming || [];
        ...
        createFloatingAim: delayedProcedure
            ...
            const status = input.aim.status
                ...
                : { state: 'open', comment: '', date: Date.now() };
            const aim = {
                id: aimId,
                text: input.aim.text,
        ...
        createSubAim: delayedProcedure
            ...
            if (process.env.NODE_ENV !== 'test') {
                generateEmbedding(childAim.text).then(vector => {
                    if (vector)
                        saveEmbedding(input.projectPath, childAimId, vector);
                });
            }
        ...
        search: delayedProcedure
            ...
            if (input.query && input.query.trim().length > 0) {
                ...
                const fallbackResults = allAims.filter(aim => aim.text.toLowerCase().includes(lowerQuery) &&
                    !results.some(r => r.id === aim.id)).map(aim => ({ ...aim, score: 0.05 })); // Assign low score to fallback matches
        ...
        searchSemantic: delayedProcedure
            ...
            for (const res of vectorResults) {
                try {
                    ...
                    if (input.status) {
                        const statuses = Array.isArray(input.status) ? input.status : [input.status];
                        if (!statuses.includes(aim.status.state))
                            match = false;
                    }
                    if (match && input.phaseId) {
                        if (!aim.committedIn.includes(input.phaseId))
                            match = false;
                    }
                    if (match) {
    ...
    project: t.router({
        onUpdate: t.procedure.subscription(() => {
            return observable((emit) => {
                const onChange = (data) => emit.next(data);
