...
function distributeCosts(aims, aimMap, values, flowValues, isDoneCost, totalCosts) {
    const costs = new Map();
    ...
    // Yes.
    const costDependencyMatrix = new Map();
    for (const parent of aims) {
        const deps = [];
    ...
    for (const parent of aims) {
        if (parent.supportingConnections) {
            for (const conn of parent.supportingConnections) {
                if (!aimMap.has(conn.aimId))
                    continue;
                if (!childToParents.has(conn.aimId))
                    childToParents.set(conn.aimId, []);
...
*/
//# sourceMappingURL=value-calculation.js.map